...
class ImageData(Dataset):
    ...
    def __init__(
        self, folder, image_size, exts=['jpg', 'jpeg', 'png', 'tiff'], 
        augment_horizontal_flip=False, 
        convert_image_to=None, in_mem=False, verbose=True
    ):
        super().__init__()
        self.folder = folder
        ...
        if in_mem:
            idx_iter = tqdm(range(0, len(self.paths)), desc='data loading') if verbose else range(0, len(self.paths))
...
class Trainer(object):
    ...
    def __init__(
        ...
        *,
        ...
        self.ds = ImageData(
            folder, self.image_size, in_mem=in_memory, 
        ...
        prompt = 'only the main process loads the data' if in_memory else ('num of the worker processes for data loading: ' + str(real_workers))
        if self.accelerator.is_main_process:
            print('\n' + prompt, flush=True, end='\n\n')
        dl = DataLoader(self.ds, batch_size=train_batch_size, shuffle=True, pin_memory=True, num_workers=real_workers)
        ...
        if self.calculate_fid:
            if not is_ddim_sampling:
                self.accelerator.print("WARNING: Robust FID computation requires a lot of generated samples and can therefore be very time consuming. Consider using DDIM sampling to save time.")
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.save_and_sample_every):
                        ...
                        with torch.inference_mode():
                            ...
                            batches = num_to_groups(self.num_samples, min(self.test_bsz, self.num_samples))
                            all_images_list = list(map(lambda n: self.ema.ema_model.sample(batch_size=n, clip_denoising=True), batches))

                        all_images = torch.cat(all_images_list, dim=0)
                        snapshot_path = str(self.results_folder / f'sample-{milestone}.png')
                        utils.save_image(all_images, snapshot_path, nrow=int(math.sqrt(self.num_samples)))
                        ...
                        plot_buffer.append(1.0 * accum_loss / self.save_and_sample_every)
                        plot_penalties.append(1.0 * accum_reg / self.save_and_sample_every)
                        plt.figure(figsize=(13, 5))
                        plt.subplot(1, 2, 1)
                        plt.plot(np.arange(1, 1 + len(plot_buffer)) * self.save_and_sample_every, plot_buffer)
                        plt.ylabel('Avg Loss', fontsize=15)
                        plt.xlabel('Training Steps', fontsize=15)
                        plt.subplot(1, 2, 2)
                        plt.plot(np.arange(1, 1 + len(plot_buffer)) * self.save_and_sample_every, plot_penalties)
